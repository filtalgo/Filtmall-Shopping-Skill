#!/usr/bin/env node
// Generated by scripts/build-clawhub-skill.js. Do not edit by hand.
(function() {
...
async function handleCart(args, opts) {
  ...
  switch (sub) {
    ...
    case 'clear': {
      if (!opts.confirm) throw new CliError('cart clear requires --confirm', 'CONFIRM_REQUIRED');
...
async function handleAddress(args, opts) {
  ...
  switch (sub) {
    ...
    case 'delete': {
      ...
      if (!opts.confirm) throw new CliError('address delete requires --confirm', 'CONFIRM_REQUIRED');
...
async function handleOrder(args, opts) {
  ...
  switch (sub) {
    ...
    case 'cancel': {
      ...
      if (!opts.confirm) throw new CliError('order cancel requires --confirm. It may cancel an unpaid order or request a full refund for a paid undelivered order.', 'CONFIRM_REQUIRED');
...
async function handleAfterSale(args, opts) {
  ...
  switch (sub) {
    ...
    case 'cancel': {
      ...
      if (!opts.confirm) throw new CliError('aftersale cancel requires --confirm', 'CONFIRM_REQUIRED');
  ...
  switch (sub) {
    ...
    case 'prepare-payment': {
      ...
      if (opts.buyerBaseUrl) {
        callArgs.buyer_base_url = opts.buyerBaseUrl;
      }
    ...
    case 'cancel': {
      const id = args[1];
      if (!id) throw new CliError('Usage: filtalgo checkout cancel <id>', 'USAGE');
...
function assertAgentGatewayPath(adapterPath) {
  ...
  if (/^https?:\/\//i.test(path)) {
    try {
      ...
      throw new CliError('Invalid adapter URL', 'INVALID_URL');
    }
  }
...
class HttpsConnectProxyAgent extends https.Agent {
  ...
  createConnection(options, callback) {
    ...
    const onData = (chunk) => {
      ...
      if (status !== 200) {
        socket.destroy();
        callback(new Error(`Proxy CONNECT failed with HTTP ${status || 'unknown'}`));
        return;
      }
...
function request(method, path, body, opts = {}) {
  return new Promise(async (resolve, reject) => {
    ...
    const mod = url.protocol === 'https:' ? https : http;
    const reqOpts = requestOptionsForUrl(url, {
      hostname: url.hostname,
      port: url.port || (url.protocol === 'https:' ? 443 : 80),
      path: url.pathname + url.search,
      method,
...
module.exports = { request, get, post, mcpCall, mcpList, getValidCredentials, _buildAdapterUrl: buildAdapterUrl };
  ...
  function load(id) {
    ...
    function localRequire(request) {
      const mapped = Object.prototype.hasOwnProperty.call(record.deps, request) ? record.deps[request] : null;
